...
\begin{itemize}
  ...
  \mpitem Query vector for a vocabulary vector (``token'') $\wv$ any time between $n_1$ to $n_2$ is
   \[
   ...
   \]
  \mpitem As vectors recede into the past, the \emph{width} of such a
  query vector \emph{shrinks}, causing dot-products among vectors in the group to
  \emph{increase} % due to the compressive time warping
  \mpitem To compensate, a forgetting factor can be used, \ie, $\hv_a(n) = \rho_n
  e^{j\Delta_n}\hv(n)$ for some $\rho_n$
\end{itemize}
\end{slide}
