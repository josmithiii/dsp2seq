...
\begin{slide}[\slideopts,toc={}]{Decaying Vector Retrieval by Inner Product}
  ...
  \]
  where $b$ is the \emph{detection threshold} for $\xv(k)$, independent of $k$ given $\|\xv(k)\|=1$

  \begin{itemize}
  \mpitem \emph{Cannot retrieve} when $p^{n-k} < b$, setting an upper limit on $n$
  \mpitem $p < b^{1/n}$ and $n < \log(b)/\log(p)$
  \mpitem Lowering $b$ extends the memory range, but allows more false detections due to ``noise''
  \mpitem ``Noise'' here is ``other random vectors in the sum''
  \end{itemize}
...
\textbf{Suggestions:}
\begin{itemize}
\mpitem \emph{Initialize biases larger than $1/N$} or close to that
...
\eqsp \zbox{\frac{n}{N}}
\eeas
assuming $\wv\perp\yv$ and $\|\wv\|=\|\xv_m\|=1$ for all $m$. Thus,
\emph{retrieval becomes unreliable when the number of summed vectors $n$ reaches the model dimension $N$.}
\begin{itemize}
...
%  \mpitem Later it will similarly be the importance of \emph{selective attention}
  \mpitem If $L$ vectors are in the sum, our Perceptron ``bias'' (detection threshold) must be higher than $L/N$
\end{itemize}
\end{slide}
...
\beas
E\left[\left(\wv^T\yv_n\right)^2\right]
&=& E\left[\left(\sum_{m=0}^n \wv^T\nu^m\xv_m\right)^2\right]
\eqsp \sum_{l=0}^n\sum_{m=0}^n E \left[ \wv^T\lambda^l\xv_l\nu^m\xv_m^T\wv \right]\\[5pt]
&=& \sum_{m=0}^n \nu^{2m} E \left[ \wv^T\xv_m\xv_m^T\wv \right]
\eqsp \sum_{m=0}^n \nu^{2m} E\left[\left(\wv^T\xv_m\right)^2\right]\\[5pt]
&=& \zbox{\frac{1}{N} \frac{1-\nu^{2(n+1)}}{1-\nu^2}}
\;\to\; \frac{1}{N} \frac{1}{1-\nu^2}\quad\mbox{(as $n\to\infty$)}
\eeas
\begin{itemize}
  \mpitem For $1/(1-\nu^2) < N$, keep $\nu < \sqrt{(N-1)/N}$
  \mpitem For $1/(1-\nu^2) < N/2$, keep $\nu < \sqrt{(N-2)/N}$
\end{itemize}
and so on.  \emph{This gives us one way to calculate a maximum feedback coefficient in RNNs}
\end{slide}
...
% MambaFormer is taken, so maybe TransMamba?
\vspace{-1em}
...
\vspace{-1.6em}
\myFigureToWidth{transmamba}{\twidth}{TransMamba}
\vspace{-2em}
\maybepause
\myFigureToWidth{cartoon-mamba-t}{0.9in}{}
\end{slide}
