...
\textbf{``Obvious'' Training Considerations for Learning Memory Duration:}
\begin{itemize}
\mpitem Initialize $\peev$ for desired initial memory duration (exponentially fading)
\mpitem Learn $\peev(\xv_n)$ as $\Imtx\cdot e^{-\Delta}\approx \Imtx -\Imtx \Delta$, and $\geev(\xv_n)$ as $\mbox{Linear}(\xv_n,\yv_n)\cdot\Delta$, where
$\Delta = \mbox{softPlus}(\mbox{parameter}(\xv_n,\Yv_n))$ (approximately gain-normalized)
\mpitem Note that \emph{gating} $\Rightarrow$ \emph{activation} (LSTM, GRU) while \emph{memory duration} $\Rightarrow$ \emph{linear projection}\\
(SSMs, Mamba) --- Structured State Space Models led to \emph{linear projection} based on cool \emph{history polynomial approximation} ideas that ultimately
went away (S4D)
\end{itemize}
...
\begin{itemize}
  ...
  \mpitem Feedback matrix $\Amtx$ has been typically \emph{diagonal} since ``S4D'' (2022)\\
