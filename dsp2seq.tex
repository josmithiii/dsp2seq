...
\maybepause
\vspace{-1em}
\textbf{Project Idea: Let's Make an Associative Memory!}
\begin{itemize}
...
\textbf{Suggestions:}
\begin{itemize}
\mpitem Use \emph{activations} for gating (LSTM, GRU)
\mpitem While activated, \emph{optionally} set \emph{memory duration} with $\peev$ (SSMs, Mamba)
\begin{itemize}
  ...
  % (approximately gain-normalized)
%\mpitem \emph{memory duration} $\Rightarrow$ \emph{linear projection}\\
  % (SSMs, Mamba) ---
  \mpitem Structured State Space Models led to \emph{linear projection} $\peev(\xv_n)$ (\emph{no activation})
  based on cool \emph{history-polynomial-approximation} ideas that ultimately went away (S4D), but \emph{linearity survives}.\\
  \mpitem Consider \emph{separate activation} multiplying feedback: $\sigma(\Lmtx\xv)\peev(\xv)$
\end{itemize}
\end{itemize}
