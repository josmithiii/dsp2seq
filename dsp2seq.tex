...
\begin{slide}[\slideopts,toc={TIIR RNNs}]{Truncated Infinite Impulse Response (TIIR) RNNs}
  ...
  \begin{itemize}
    ...
    \end{align*}
    \vspace{-1em}
    \mpitem \textbf{Problem:} Accumulating floating-point round-off error (variance increases linearly)
  \end{itemize}
\end{slide}

\begin{slide}[\slideopts,toc={TIIR Sliding Window}]{TIIR RNN with Sliding-Window Memory and Linear RoPE}
\myFigureToWidth{tiir-rnn}{\twidth}{}
\end{slide}

\begin{slide}[\slideopts,toc={TIIR Resets}]{Ping-Ponging Periodically Resetting RNNs (Classic TIIR Trick)}
  \vspace{-1em}
  Needed when there is a \emph{forgetting factor} $p$ in the hidden-state feedback:
  \vspace{-2em}
\maybepause
...
\begin{slide}[\slideopts,toc={Plan}]{Next Steps}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \mpitem Truncated Infinite Impulse Response (TIIR) sliding-window memory + linear RoPE
