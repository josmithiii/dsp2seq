...
\begin{itemize}
  ...
  \mpitem Query vector for a vocabulary vector (``token'') $\wv$ between $n_1$ to $n_2$ is
   \[
   \qv = \sum_{n=n_1}^{n_2}e^{jn \Delta}\wv
   = \wv \frac{e^{j(n_2+1)\Delta} - e^{jn_1\Delta}}{e^{j\Delta}-1}
   ...
   \]

  \mpitem As vectors recede into the past, dot-products among them
  \emph{increase} due to the compressive time warping. To compensate
  for this, a forgetting factor can be used, \ie, $\hv_a(n) = \rho_n
  e^{j\Delta_n}\hv(n)$ for some choice of $\rho_n$.
\end{itemize}
\end{slide}
...
\begin{itemize}
  ...
  and $\cos(\theta)=b$
  \mpitem In WRoPE, the angular rotation decreases as $1/t$ or the like
  \mpitem ...
\end{itemize}

This causes inner products to increase as vectors get squeezed together.
...
%    \begin{enumerate}
	...
	(data size doubled: $\xv \to (\xv,\zv)$) --- the zero imaginary part reserves complex angle for [W]RoPE
      \mpitem Use \emph{quaternions} for two more angles $\Rightarrow$ data size quadrupled: $\xv \to (\xv,\zv,\zv,\zv)$\\
      % https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation
      $\hv_a(n) = \qb\hv(n)$, where, from \htmladdnormallink{Wikipedia}{https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation}:\\
      $
      % \displaystyle
