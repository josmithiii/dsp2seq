...
Contribution from input $\xv(n-j)$ to the nonlinear FIR sum for output $\yv(n-k)$ is
$[\xv(n-k)^T \xv(n-j)]\xv(n-j)$,
\maybepause
or more generally $(Q_k^T K_j)\xv(n-j)$, where
\begin{itemize}
...
down-projections in \emph{multi-head} mode), replace $\xv(n-j)$ in the
attention sum with a learned \emph{value vector} $V_j[\xv(n-j)]$\\
%
\maybepause
$\Rightarrow$\\
%
Relevance of input $j$ to output $k$ is $\propto (Q_k^T K_j)V_j$, where
all three vectors are learned projections of the input $\xv$ at positions
$j$ and $k$, for all available $j$ and $k$ (``Transformer'')
