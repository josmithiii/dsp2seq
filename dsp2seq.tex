...
\vspace{-1em}
\textbf{Vectorize it!}
\textbf{One Unit Delay (for Vectors) can be an Associative Memory:}
%\textbf{Idea: Let's Make an Associative Memory!}
\begin{itemize}
\mpitem Let $x(n)$ be a \emph{long vector} $\xv(n)\in\RN$ representing any ``label''
...
\mpitem Let $\xv(n)$ be \textbf{embedded vectors} (\eg, \tx{word2vec}) so that \emph{closeness = similarity}
\mpitem Retrieve similar vectors using a \emph{matched inner product} $\wv^T\xv > b$,\\
        for some suitable threshold $b$ (Hey!  A simulated neuron! (``Perceptron'' [1957]))
\end{itemize}
\end{slide}
...
\begin{slide}[\slideopts,toc={Channel Mixing}]{Input Gating with ``Component Mixing''}
...
%\vspace{-1em}
  Hook up an MLP to extract information from the vector memory:
%\vspace{-1em}
...
\mpitem Final linear layer typically projects back to the model dimension
\mpitem Skip connection normally provided, bypassing the ``residual stream''
\end{itemize}
\end{slide}
...
\begin{slide}[\slideopts,toc={MLP}]{MultiLayer Perceptrons (MLP)}
  ...
  \textbf{Idea:} The outputs from a Perceptron Array can be further processed by \emph{another Perceptron Array}
  \begin{itemize}
...
\begin{itemize}
  ...
  \mpitem Piecewise-linear polytopes in the model dimension become smoother (good for audio effects)
