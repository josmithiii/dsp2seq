...
\beas
E\left[\left(\wv^T\yv_n\right)^2\right]
&=& E\left[\left(\sum_{m=0}^n \wv^Tp^m\xv_m\right)^2\right]
\eqsp \sum_{l=0}^n\sum_{m=0}^n E \left[ \wv^T\lambda^l\xv_lp^m\xv_m^T\wv \right]\\[5pt]
&=& \sum_{m=0}^n p^{2m} E \left[ \wv^T\xv_m\xv_m^T\wv \right]
\eqsp \sum_{m=0}^n p^{2m} E\left[\left(\wv^T\xv_m\right)^2\right]\\[5pt]
