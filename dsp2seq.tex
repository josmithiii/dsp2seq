...
\begin{itemize}
  ...
  \mpitem RoPE query vector for a vocabulary vector (``token'') $\wv$ any time between $n_1$ to $n_2$ is
   ...
   \]
  \mpitem In WRoPE, as vectors recede into the past, the \emph{angular width} of such a
  query vector \emph{shrinks} due to time-warping, causing dot-products among vectors in the group to
  \emph{increase}, making them look closer together in time
  \mpitem To compensate, a forgetting factor can be used, \ie, $\hv_a(n) = \rho_n
  e^{j\Delta_n}\hv(n)$ for some $\rho_n$
  % \mpitem [Exact calculation of $\rho_n$ for $\Delta_n = \Delta_0/(1+n)$]
\end{itemize}
\end{slide}
...
\vspace{-2em}
\myFigureToWidth{cartoon-mamba-t}{0.8in}{}
\end{slide}
